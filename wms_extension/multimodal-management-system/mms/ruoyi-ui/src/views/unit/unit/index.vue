...
<script>
import { listUnit, getUnit, delUnit, addUnit, updateUnit } from "@/api/unit/unit";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除单位编号为"' + uids + '"的数据项？').then(function() {
        return delUnit(uids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('unit/unit/export', {
        ...this.queryParams
