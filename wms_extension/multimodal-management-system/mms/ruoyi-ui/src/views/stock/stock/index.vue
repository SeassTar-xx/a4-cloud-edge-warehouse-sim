<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="已分配量" prop="assignMount">
        <el-input
          v-model="queryParams.assignMount"
          placeholder="请输入已分配量"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-table v-loading="loading" :data="stockList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="已分配量" align="center" prop="assignMount" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="已分配量" prop="assignMount">
          <el-input v-model="form.assignMount" placeholder="请输入已分配量" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        assignMount: [
          { required: true, message: "已分配量不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除初级库存录入编号为"' + ids + '"的数据项？').then(function() {
        return delStock(ids);
      }).then(() => {
        this.getList();
